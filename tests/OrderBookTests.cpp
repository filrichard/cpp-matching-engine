...
TEST_CASE("OrderBook: trade prints at the resting order's price, not the aggressor's", "[orderbook]") {
    OrderBook book("TEST");
...
TEST_CASE("OrderBook: IOC order fills what it can and discards the remainder", "[orderbook][tif]") {
    OrderBook book("TEST");
    book.addOrder(Order::makeLimit(1, kClientA, "TEST", Side::Sell, 100, 5));
...
TEST_CASE("OrderBook: FOK order is rejected outright if it can't be fully filled", "[orderbook][tif]") {
    OrderBook book("TEST");
    book.addOrder(Order::makeLimit(1, kClientA, "TEST", Side::Sell, 100, 5));
...
TEST_CASE("OrderBook: market order matches without needing a price and never rests", "[orderbook]") {
    OrderBook book("TEST");
    book.addOrder(Order::makeLimit(1, kClientA, "TEST", Side::Sell, 100, 5));
...
TEST_CASE("OrderBook: market order with insufficient liquidity fills what exists and discards the rest", "[orderbook]") {
    OrderBook book("TEST");
    book.addOrder(Order::makeLimit(1, kClientA, "TEST", Side::Sell, 100, 5));
...
TEST_CASE("OrderBook: cancelling a resting order removes it from the book", "[orderbook][cancel]") {
    OrderBook book("TEST");
...
TEST_CASE("OrderBook: cancelling an unknown or already-inactive order returns false", "[orderbook][cancel]") {
    OrderBook book("TEST");
...
TEST_CASE("OrderBook: cancelling one order at a price level leaves the others intact", "[orderbook][cancel]") {
    OrderBook book("TEST");
...
TEST_CASE("MatchingEngine: submitting to an unregistered symbol throws", "[engine]") {
    ...
    CHECK_THROWS_AS(
        engine.submitLimitOrder(kClientA, "MSFT", Side::Buy, 100, 10),
...
TEST_CASE("MatchingEngine: a crossing order produces trades in the result and via the callback", "[engine]") {
    MatchingEngine engine;
    engine.addSymbol("AAPL");

    std::vector<Trade> observed;
    engine.setTradeCallback([&observed](const Trade& trade) { observed.push_back(trade); });
