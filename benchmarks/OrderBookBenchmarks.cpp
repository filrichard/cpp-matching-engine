...
static void BM_OrderBook_RestingInsert ( benchmark::State& state )
{
    ...
    {
        book.addOrder ( Order::makeLimit ( id++, kClient, "BENCH", Side::Sell, 100'000 + i, 10 ) );
    }
    ...
    for ( auto _ : state )
    {
        auto trades = book.addOrder (
            Order::makeLimit ( id, kClient, "BENCH", Side::Sell, probe_price, 10 ) );
...
{
    OrderBook book ( "BENCH" );
    book.addOrder ( Order::makeLimit ( 1, kClient, "BENCH", Side::Sell, 100, 1'000'000'000ULL ) );

    OrderId next_id = 2;
    for ( auto _ : state )
    {
        auto trades =
            book.addOrder ( Order::makeLimit ( next_id++, kClient, "BENCH", Side::Buy, 100, 1 ) );
        benchmark::DoNotOptimize ( trades );
    }
...
static void BM_OrderBook_SweepLevels ( benchmark::State& state )
{
    ...
    auto populate = [&] {
        ...
        {
            book.addOrder (
                Order::makeLimit ( next_id++, kClient, "BENCH", Side::Sell, 100 + i, 1 ) );
        }
    };
    populate();
    ...
    {
        auto trades = book.addOrder (
            Order::makeLimit ( next_id++, kClient, "BENCH", Side::Buy, 100 + levels - 1, levels ) );
        benchmark::DoNotOptimize ( trades );
...
static void BM_MatchingEngine_SubmitNonCrossing ( benchmark::State& state )
{
    ...
    for ( auto _ : state )
    {
        auto result = engine.submitLimitOrder ( kClient, "BENCH", Side::Sell, probe_price, 10 );
